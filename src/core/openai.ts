...
function responsesHistoryFramingTokens(
  ...
  for (let i = 0; i < plan.segments.length; i++) {
    ...
    for (const part of content) {
      ...
      if (typeof text === 'string') total += gptTextTokens(text);
    }
  }
  return total;
}
...
async function responsesHistorySegmentShas(
  ...
  return Promise.all(plan.segments.map(async (_segment, index) => {
    // Hash the exact synthetic Responses item, including intro/outro, factsheet
    // and every image data URL. PNG-only hashes are insufficient: a moving
    // combined factsheet can bust an otherwise byte-identical image prefix.
...
function cacheAllowsCodexHistory(
  ...
  if (!hint) {
    // A daemon restart does not mean OpenAI's prefix cache is cold. Observe one
    // native request first instead of gambling a large cache miss on an image
    // transition whose economics are not yet grounded.
...
async function applyResponsesHistoryCollapse(
  ...
  // Function-tool metrics stay backwards-compatible while Codex custom-tool
  // state gets its own explicit accounting instead of disappearing into `other`
  // and acting as an opaque history barrier.
  ...
  // Composition telemetry above is useful even on abstention. Renderer loss is
  // checked before any economics/cache decision because semantic safety wins.
  if (historyRenderLossy(info, o, plan)) return false;
  ...
  if (o.codexOptimization) {
    const m = codexHistoryMateriality(plan, req, profile);
    rc.historyCandidateRawSaving = m.rawSaving;
    rc.historyCandidateRawSavingPct = m.ratio * 100;
    rc.historyCandidateSavingPerImage = m.perImage;
    ...
    if (
      m.rawSaving < minTokens
      || m.ratio < minRatio
      || m.perImage < minPerImage
    ) {
    ...
    const hint = getCodexCacheHint(info.firstUserSha8);
  ...
  // The candidate has now cleared semantic, raw materiality and (on Codex)
  // warm-prefix stability gates. Only now credit it to transform economics.
  foldGptHistory(info, req.model, plan);
  ...
  for (let segmentIndex = 0; segmentIndex < plan.segments.length; segmentIndex++) {
    ...
    for (const part of content) {
      ...
      if (typeof text === 'string') {
        info.nativeInjectedTokens = (info.nativeInjectedTokens ?? 0) + gptTextTokens(text);
      }
    }
...
export async function transformOpenAIResponses(
  ...
  // Stable Codex trajectory key. Compute it before any history-only early return
  // so the dedicated route can make cache-preservation decisions even when the
  // static slab is intentionally forbidden by coding-safe.
