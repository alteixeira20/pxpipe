...
## Remaining wiring audit

Current event rows already carry most token and usage buckets, but serialized
original-request bytes and a measured PXPipe-only latency component are not yet
available as first-class event fields. Those measurements must be added at the
proxy boundary before dashboard rollups can claim byte reduction or proxy
